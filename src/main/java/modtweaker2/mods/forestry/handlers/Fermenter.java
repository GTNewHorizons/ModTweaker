...
public class Fermenter {
    ...
    /**
     * Adds a fermenter recipe. Amount of fluid output is calculated: fermentationValue * fluidOutputModifier Note: the
     * actual consumption of fluid input depends on the fermentation fuel
     * 
     ...
     */
    @ZenMethod
    public static void addRecipe(ILiquidStack fluidOutput, IItemStack resource, ILiquidStack fluidInput,
            int fermentationValue, float fluidOutputModifier) {
        MineTweakerAPI.apply(
                new Add(
                        new FermenterRecipe(
                                ...
                                fermentationValue,
                                fluidOutputModifier,
                                getFluid(fluidOutput),
                                toFluid(fluidInput))));
    }

    @Deprecated
    @ZenMethod
    public static void addRecipe(IItemStack resource, ILiquidStack fluidInput, int fermentationValue,
            float fluidOutputModifier, ILiquidStack fluidOutput) {
        MineTweakerAPI.apply(
                new Add(
                        new FermenterRecipe(
                                toStack(resource),
    ...
    public static void removeRecipe(IIngredient input) {
        ...
        for (IFermenterRecipe recipe : RecipeManagers.fermenterManager.recipes()) {
            ...
            if (recipe != null && recipe.getResource() != null
                    && matches(input, toILiquidStack(recipe.getFluidResource()))) {
                recipes.add(recipe);
            }
        }
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", Fermenter.name, input.toString()));
        }
    }
    ...
    /**
     * Adds fermenter fuel. Note: the actual consumption of fluid input depends on the fermentation fuel
     * 
