...
public class ThermionicFabricator {
    ...
    public static void addSmelting(int fluidOutput, IItemStack itemInput, int meltingPoint) {
        // The machines internal tank accept only liquid glass, therefor this function only accept the amount and
        // hardcode the fluid to glass
        MineTweakerAPI.apply(
                new AddSmelting(
                        new FabricatorSmeltingRecipe(
                                toStack(itemInput),
                                FluidRegistry.getFluidStack("glass", fluidOutput),
                                meltingPoint)));
    }
     ...
     */
    @ZenMethod
    public static void addCast(IItemStack output, IIngredient[][] ingredients, int fluidInput,
            @Optional IItemStack plan) {
    ...
    /*
     * Implements the actions to add a recipe Since the machine has two crafting Steps, this is a constructors for both
     */
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format(
                            ...
                            ThermionicFabricator.nameSmelting,
                            itemInput.toString()));
        }
    }
