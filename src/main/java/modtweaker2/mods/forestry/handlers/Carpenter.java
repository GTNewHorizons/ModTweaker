...
public class Carpenter {
     ...
     */
    @ZenMethod
    public static void addRecipe(IItemStack output, IIngredient[][] ingredients, int packagingTime,
            @Optional IItemStack box) {
        IDescriptiveRecipe craftRecipe = new DescriptiveRecipe(
                3,
                3,
     ...
     */
    @ZenMethod
    public static void addRecipe(IItemStack output, IIngredient[][] ingredients, ILiquidStack fluidInput,
            int packagingTime, @Optional IItemStack box) {
        IDescriptiveRecipe craftRecipe = new DescriptiveRecipe(
                3,
                3,
                toShapedObjects(ingredients),
    ...
    @Deprecated
    @ZenMethod
    public static void addRecipe(int packagingTime, ILiquidStack fluidInput, IItemStack[] ingredients, IItemStack box,
            IItemStack output) {
        IDescriptiveRecipe craftRecipe = new DescriptiveRecipe(
                3,
                3,
    ...
    public static void removeRecipe(IIngredient output, @Optional IIngredient liquid) {
        ...
        for (ICarpenterRecipe recipe : RecipeManagers.carpenterManager.recipes()) {
            if (recipe != null) {
                ...
                if (recipeResult != null && matches(output, toIItemStack(recipeResult))) {
                    if (liquid != null) {
                        if (matches(liquid, toILiquidStack(recipe.getFluidResource()))) recipes.add(recipe);
                    } else {
                        recipes.add(recipe);
                    }
                }
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", Carpenter.name, output.toString()));
        }
    }
