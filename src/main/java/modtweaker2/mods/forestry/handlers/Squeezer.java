...
public class Squeezer {
    ...
    public static void addRecipe(ILiquidStack fluidOutput, int timePerItem, IItemStack[] ingredients) {
        MineTweakerAPI
                .apply(new Add(new SqueezerRecipe(timePerItem, toStacks(ingredients), toFluid(fluidOutput), null, 0)));
    }
     ...
     */
    @ZenMethod
    public static void addRecipe(ILiquidStack fluidOutput, WeightedItemStack itemOutput, IItemStack[] ingredients,
            int timePerItem) {
        MineTweakerAPI.apply(
                new Add(
                        new SqueezerRecipe(
                                timePerItem,
                                toStacks(ingredients),
                                toFluid(fluidOutput),
                                toStack(itemOutput.getStack()),
                                itemOutput.getChance())));
    }

    @ZenMethod
    @Deprecated
    public static void addRecipe(int timePerItem, IItemStack[] resources, ILiquidStack liquid, IItemStack remnants,
            int chance) {
        MineTweakerAPI.apply(
                new Add(
                        new SqueezerRecipe(
            ...
            if (r != null && r.getFluidOutput() != null && matches(liquid, toILiquidStack(r.getFluidOutput()))) {
                ...
                if (ingredients != null) {
                    ...
                    for (int i = 0; i < ingredients.length; i++) {
                        if (matches(ingredients[i], toIItemStack(r.getResources()[i]))) matched = true;
                        else {
                            matched = false;
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format(
                            ...
                            Squeezer.name,
                            LogHelper.getStackDescription(liquid)));
        }
    }
