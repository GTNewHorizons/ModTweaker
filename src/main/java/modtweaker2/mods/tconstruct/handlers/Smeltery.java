...
public class Smeltery {
    ...
    public static void addAlloy(ILiquidStack output, ILiquidStack[] input) {
        ...
        MineTweakerAPI.apply(
                new AddAlloy(new AlloyMix(toFluid(output), new ArrayList<FluidStack>(Arrays.asList(toFluids(input))))));
    }
    ...
    public static void removeAlloy(IIngredient output) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logError(
                    String.format("No %s recipes found for %s. Command ignored!", nameAlloy, output.toString()));
        }
    ...
    public static void addMelting(IIngredient input, ILiquidStack output, int temp, @Optional IItemStack block) {
        ...
        for (IItemStack in : input.getItems()) {
            if (block == null && !isABlock(toStack(in))) {
                LogHelper.logWarning(
                        String.format(
                                "Item %s is not a block and no block renderer is provided for %s recipe. Input ignored!",
                                in.toString(),
                                nameMelting));
            } else {
                recipes.add(
                        new MeltingRecipe(
                                ...
                                toFluid(output),
                                temp,
                                block == null ? toStack(in) : toStack(block)));
            }
        }
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logError(
                    String.format("No %s recipes could be added for input %s.", nameMelting, input.toString()));
        }
    }
    ...
    private static class AddMelting extends BaseListAddition<MeltingRecipe> {
        ...
        public void apply() {
            for (MeltingRecipe recipe : recipes) {
                tconstruct.library.crafting.Smeltery.addMelting(
                        recipe.input,
                        recipe.getRendererBlock(),
    ...
    public static void removeMelting(IIngredient input) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", nameMelting, input.toString()));
        }
    }
    ...
    public static void removeFuel(IIngredient input) {
        ...
        for (Entry<Fluid, Integer[]> fuel : TConstructHelper.fuelList.entrySet()) {
            if (fuel != null && fuel.getKey() != null
                    && matches(input, toILiquidStack(new FluidStack(fuel.getKey(), 1)))) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe for %s found. Command ignored!", Smeltery.nameFuel, input.toString()));
        }
    }
